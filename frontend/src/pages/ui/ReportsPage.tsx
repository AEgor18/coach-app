...
export const ReportsPage: React.FC = () => {
  ...
  const fetchReports = async () => {
    try {
      setLoading(true);
      const res = await getAllReports(localStorage.getItem("access_token")!);
  ...
  const handleAddReport = async (reportData: ReportFormData) => {
    try {
      setLoading(true);
      await createReport(localStorage.getItem("access_token")!, reportData);
          ...
          <Grid item xs={12} sm={6} md={4} key={report.id}>
            ...
            >
              ...
              <Box sx={{ mb: 2 }}>
                ...
                >
                  Период: {new Date(report.start_date).toLocaleDateString()} -{" "}
                ...
                >
                  Создан: {new Date(report.created_date).toLocaleDateString()}
                </Typography>
              </Box>
