...
import type { ReportFormData, ReportData } from "../../../types/types";
...
export const EditReportModal: React.FC<EditReportModalProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.trainings < 0) {
      newErrors.trainings = "Количество тренировок не может быть отрицательным";
    }

    if (formData.skips < 0) {
      newErrors.skips = "Количество пропусков не может быть отрицательным";
    }
  ...
  const handleChange =
    ...
    (event: React.ChangeEvent<HTMLInputElement | { value: unknown }>) => {
      ...
      if (
        ["attendance", "trainings", "skips", "participants"].includes(field)
      ) {
        if (value === "") {
          setFormData((prev) => ({
            ...prev,
