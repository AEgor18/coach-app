...
export const EditNutritionModal: React.FC<EditNutritionModalProps> = ({
  ...
  useEffect(() => {
    if (nutrition && athletes.length > 0) {
      const athleteIds = nutrition.athletes.map((athlete) => athlete.id);
      ...
      setFormData({
        ...
        period_weeks: nutrition.period_weeks,
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.calories < 500 || formData.calories > 10000) {
      newErrors.calories = "Калорийность должна быть от 500 до 10000 ккал";
    }
  ...
  const handleChange =
    ...
    (event: React.ChangeEvent<HTMLInputElement | { value: unknown }>) => {
      ...
      if (
        ["calories", "protein", "fats", "carbs", "period_weeks"].includes(field)
      ) {
        if (value === "") {
          setFormData((prev) => ({
            ...prev,
