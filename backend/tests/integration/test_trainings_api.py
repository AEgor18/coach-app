...
class TestTrainingsEndpoints:
    ...
    def test_create_training_plan_success(self, authorized_client, db_session, test_coach):
        athlete = create_athlete(db_session, AthleteCreate(
            name="Для тренировки", sport_type=SportType.RUNNING,
            age=25, phone="+79990009001", progress=0
    ...
    def test_create_training_plan_validation_error(self, authorized_client):
        payload = {"date": "invalid", "title": "", "training_type": "Кардио",
                   "duration": 45, "skill_level": "Средний"}
        response = authorized_client.post("/api/trainings/plans", json=payload)
    ...
    def test_update_training_plan_success(self, authorized_client, db_session, test_coach):
        ...
        plan = create_training_plan(db_session, TrainingPlanCreate(
            date=date.today(), title="Исходный", training_type="Силовые",
            duration=30, skill_level="Начальный", description="", athlete_ids=[athlete.id]
        ), coach_id=test_coach.id)
    ...
    def test_update_training_status_success(self, authorized_client, db_session, test_coach):
        ...
        plan = create_training_plan(db_session, TrainingPlanCreate(
            date=date.today(), title="Статус тест", training_type="Кардио",
            duration=45, skill_level="Средний", description="", athlete_ids=[athlete.id]
        ), coach_id=test_coach.id)
    ...
    def test_delete_training_plan_success(self, authorized_client, db_session, test_coach):
        ...
        from schemas.trainings import TrainingPlanCreate
        plan = create_training_plan(db_session, TrainingPlanCreate(
            date=date.today(), title="На удаление", training_type="Кардио",
            duration=45, skill_level="Средний", description="", athlete_ids=[]
        ), coach_id=test_coach.id)
    ...
    def test_training_plan_isolation_by_coach(self, authorized_client, db_session):
        from crud.trainings import create_training_plan
        from schemas.trainings import TrainingPlanCreate
        create_training_plan(db_session, TrainingPlanCreate(
            date=date.today(), title="Чужой план", training_type="Кардио",
            duration=45, skill_level="Средний", description="", athlete_ids=[]
        ), coach_id=999)
