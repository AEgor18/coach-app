...
from models.athletes import SportType, AthleteStatus
...
class TestAthletesEndpoints:
    ...
    def test_update_athlete_success(self, authorized_client, db_session, test_coach):
        ...
        athlete = create_athlete(db_session, AthleteCreate(
            name="Исходный", sport_type=SportType.SWIMMING,
            age=20, phone="+79990003001", progress=10
        ), coach_id=test_coach.id)
        
        response = authorized_client.put(
            f"/api/athletes/{athlete.id}",
            json={"name": "Обновлённый", "progress": 90}
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_delete_athlete_success(self, authorized_client, db_session, test_coach):
        ...
        from schemas.athletes import AthleteCreate
        athlete = create_athlete(db_session, AthleteCreate(
            name="На удаление", sport_type=SportType.YOGA,
            age=22, phone="+79990003002", progress=0
        ), coach_id=test_coach.id)
    ...
    def test_athlete_isolation_by_coach(self, authorized_client, db_session):
        ...
        create_athlete(db_session, AthleteCreate(
            name="Чужой", sport_type=SportType.RUNNING,
            age=25, phone="+79990003003", progress=0
        ), coach_id=999)
...
class TestAthletesPaginationFiltering:
    ...
    def test_pagination_params(self, authorized_client, db_session, test_coach):
        ...
        for i in range(15):
            create_athlete(db_session, AthleteCreate(
                ...
                age=20 + i, phone=f"+79990004{i:03d}", progress=i * 5
            ), coach_id=test_coach.id)
    ...
    def test_filter_by_age_range(self, authorized_client, db_session, test_coach):
        ...
        for age in [18, 22, 25, 30, 35]:
            create_athlete(db_session, AthleteCreate(
                name=f"Age{age}", sport_type=SportType.YOGA,
                age=age, phone=f"+79990005{age:03d}", progress=0
            ), coach_id=test_coach.id)
    ...
    def test_search_filter(self, authorized_client, db_session, test_coach):
        ...
        for name in ["Александр", "Алексей", "Мария"]:
            create_athlete(db_session, AthleteCreate(
                ...
                age=25, phone=f"+79990006{hash(name) % 1000:03d}", progress=0
            ), coach_id=test_coach.id)
    ...
    def test_sorting(self, authorized_client, db_session, test_coach):
        ...
        for name, age in [("Zoe", 20), ("Alex", 30), ("Mike", 25)]:
            create_athlete(db_session, AthleteCreate(
                name=name, sport_type=SportType.RUNNING,
                age=age, phone=f"+79990007{hash(name) % 1000:03d}", progress=0
            ), coach_id=test_coach.id)
