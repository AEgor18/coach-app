...
class TestNutritionEndpoints:
    ...
    def test_create_nutrition_plan_success(self, authorized_client, db_session, test_coach):
        athlete = create_athlete(db_session, AthleteCreate(
            ...
            age=25, phone="+79990008001", progress=0
    ...
    def test_create_nutrition_plan_validation_error(self, authorized_client):
        payload = {"title": "", "nutrition_type": "набор массы", "calories": 2500,
                   "protein": 150, "fats": 70, "carbs": 300, "period_weeks": 8,
    ...
    def test_update_nutrition_plan_success(self, authorized_client, db_session, test_coach):
        ...
        plan = create_nutrition_plan(db_session, NutritionPlanCreate(
            title="Исходный", nutrition_type="поддержание", calories=2000,
            protein=100, fats=60, carbs=250, period_weeks=4,
            breakfast="Завтрак", lunch="Обед", dinner="Ужин", athlete_ids=[athlete.id]
        ), coach_id=test_coach.id)
    ...
    def test_update_nutrition_status_success(self, authorized_client, db_session, test_coach):
        ...
        plan = create_nutrition_plan(db_session, NutritionPlanCreate(
            title="Статус тест", nutrition_type="поддержание", calories=2000,
            protein=100, fats=60, carbs=250, period_weeks=4,
            breakfast="Завтрак", lunch="Обед", dinner="Ужин", athlete_ids=[athlete.id]
        ), coach_id=test_coach.id)
    ...
    def test_delete_nutrition_plan_success(self, authorized_client, db_session, test_coach):
        ...
        from schemas.nutrition import NutritionPlanCreate
        plan = create_nutrition_plan(db_session, NutritionPlanCreate(
            title="На удаление", nutrition_type="поддержание", calories=2000,
            protein=100, fats=60, carbs=250, period_weeks=4,
            breakfast="Завтрак", lunch="Обед", dinner="Ужин", athlete_ids=[]
        ), coach_id=test_coach.id)
    ...
    def test_nutrition_plan_isolation_by_coach(self, authorized_client, db_session):
        from crud.nutrition import create_nutrition_plan
        from schemas.nutrition import NutritionPlanCreate
        create_nutrition_plan(db_session, NutritionPlanCreate(
            title="Чужой план", nutrition_type="поддержание", calories=2000,
            protein=100, fats=60, carbs=250, period_weeks=4,
            breakfast="Завтрак", lunch="Обед", dinner="Ужин", athlete_ids=[]
        ), coach_id=999)
