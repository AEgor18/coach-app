...
from crud.reports import create_report, get_report, update_report, get_reports
...
class TestReportsCRUD:
    ...
    def test_report_filtered_by_coach(self, db_session: Session, test_coach):
        create_report(db_session, ReportCreate(
            title="Мой отчёт", start_date=date(2024,1,1), end_date=date(2024,1,31),
            created_date=date(2024,2,1), attendance=90, trainings=10, skips=1, participants=5
        ), coach_id=test_coach.id)
        
        create_report(db_session, ReportCreate(
            title="Чужой отчёт", start_date=date(2024,1,1), end_date=date(2024,1,31),
            created_date=date(2024,2,1), attendance=80, trainings=8, skips=2, participants=3
        ), coach_id=999)
