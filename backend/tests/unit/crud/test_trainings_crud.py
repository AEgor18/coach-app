...
from crud.trainings import create_training_plan, get_training_plan, update_training_plan, update_training_status
from schemas.trainings import TrainingPlanCreate, TrainingPlanUpdate
from models.athletes import SportType
from models.trainings import TrainingStatus, TrainingType, SkillLevel
...
class TestTrainingsCRUD:
    ...
    def test_create_training_plan_success(self, db_session: Session, test_coach):
        athlete = create_athlete(db_session, AthleteCreate(
            ...
            age=25, phone="+79990002001", progress=0
    ...
    def test_update_training_status(self, db_session: Session, test_coach):
        ...
        plan = create_training_plan(db_session, TrainingPlanCreate(
            date=date.today(), title="Тест", training_type="Силовые",
            duration=30, skill_level="Начальный",
            description="", athlete_ids=[athlete.id]
        ), test_coach.id)
