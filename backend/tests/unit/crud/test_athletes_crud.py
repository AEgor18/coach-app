...
class TestGetAthletes:
    ...
    def test_get_athletes_filter_by_sport_type(self, db_session: Session, test_coach):
        for idx, sport in enumerate(
            [SportType.SWIMMING, SportType.RUNNING, SportType.SWIMMING], start=20
        ):
            create_athlete(
                db_session,
                AthleteCreate(
                    name=f"Спортсмен {sport.value}",
                    sport_type=sport,
                    age=20,
                    phone=make_phone(idx),
                    progress=0,
...
class TestUpdateAthleteStatus:
    def test_update_status_success(self, db_session: Session, test_coach):
        ...
        result = update_athlete_status(db_session, athlete.id, AthleteStatus.INJURED)
